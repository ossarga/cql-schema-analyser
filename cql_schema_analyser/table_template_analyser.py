...
class TableTemplateAnalyser:
    ...
    @staticmethod
    def __get_field_hash(fields_list, key_dict):
        fields_hash = hashlib.md5()
        fields_list.sort()
        encoded = json.dumps(fields_list).encode()
        fields_hash.update(encoded)

        key_dict['partition'].sort()
        key_dict['clustering'].sort()
        encoded = json.dumps(key_dict, sort_keys=True).encode()
        fields_hash.update(encoded)
        return fields_hash.hexdigest()
    ...
    def __add_table_template_definition(self, dom_object, fields_hash, props_hash, match_percent):
        self.template_definitions[fields_hash]['variants'].append(props_hash)
        self.template_definitions[fields_hash][props_hash] = {
            'fields': dom_object['attributes']['fields'],
    ...
    def catalog_table_definition(self, dom_object):
        fields_hash = self.__get_field_hash(dom_object['attributes']['fields'], dom_object['attributes']['key'])
        props_hash = self.__get_properties_hash(dom_object['attributes']['properties'])

        if fields_hash in self.template_definitions:
            if props_hash in self.template_definitions[fields_hash]:
                self.template_definitions[fields_hash][props_hash]['occurrences'].append({
                    'name': dom_object['name'],
                    'match': 1
                })
            else:
                ref_props_hash = self.template_definitions[fields_hash]['variants'][0]
                ref_props = copy.deepcopy(self.template_definitions[fields_hash][ref_props_hash]['properties'])
                ...
                if match_percent > 0.5:
                    self.template_definitions[fields_hash][ref_props_hash]['occurrences'].append({
                        'name': dom_object['name'],
                        'match': match_percent
                    })
                else:
                    self.__add_table_template_definition(dom_object, fields_hash, props_hash, match_percent)
        else:
            self.template_definitions[fields_hash] = {
                'variants': [],
            }
            self.__add_table_template_definition(dom_object, fields_hash, props_hash, 1)
